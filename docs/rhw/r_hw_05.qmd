...
1. Create 10000 samples, of size n, from the original population `samp` using `map(1:10000, ~ f)` and `sample()`. Remember to sample with replacement, `replace = TRUE` inside `sample()`, and make sure each sample is of the same size as the population, `n = 25`.
...
2. The data for this type of problem is a series of successes and failures. We will encode 0 as a failure and 1 as a success. The way that we can simulate a series of successes and failures that have a common probability of success $p$ is with a binomial distribution. To be specific, if I wanted simulate a sample of 25 successes and failures with the probability of success being $0.25$, I would write `rbinom(n = 25, 1, p = 0.25)`. I want you to simulate 10000 samples of size $n = 25$ from a binomial distribution with probability of success $p = 0.25$. You'll want to use something like the `map(1:10000, ~ f)` function that you've used in previous homeworks. The result should be a list of vectors that contain a bunch of 0's and 1's.
